#!/usr/bin/env python3
"""Run evaluate_codenet_python.py over disjoint archive shards in parallel.

Each worker owns one modulo shard of the Python members in archive order.  The
requested clean-file count is divided across the workers, so no source file is
evaluated twice.  This intentionally trades the evaluator's exact global
"first N clean files" boundary for a simple one-pass-per-worker partition.
"""
...
class Aggregate:
    ...
    def summary(self, return_codes: Mapping[int, int]) -> dict[str, object]:
        ...
        return {
            ...
            "population": (
                "all eligible statements in statically sharded archive-order "
                "ty-clean files"
            ),
...
def main(arguments: Sequence[str] | None = None) -> int:
    ...
    start = {
        ...
        "archive": str(base_options.archive),
        "files": serial.files,
        "precision_samples": serial.precision_samples,
        "max_samples": serial.max_samples,
        "partition": "python-member-index modulo workers",
    }
    if serial.jsonl:
        ...
        print(
            ...
            f"target_files={serial.files}; partition=member-index-modulo",
            flush=True,
        )
